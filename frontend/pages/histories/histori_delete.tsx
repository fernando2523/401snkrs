...
export default function Refund() {
    ...
    async function getdatarefund(tanggal: any, Store: any, Role: any, area: any, Query: any) {
        await axios
            .post("https://api.epseugroup.com/v1/getdeleteorder", {
                tanggal: tanggal,
                store: Store,
                role: Role,
                area: area,
    ...
    useEffect(() => {
        getdatarefund(date, Store, Role, area, Query);
        getstore(Role, area);
    }, []);
                ...
                <div key={index}>
                    <div className="mb-2  group hover:shadow-lg ">
                        <div className="bg-white flex flex-row h-[40px] rounded-lg items-center px-0 text-[9px]">
                            ...
                            <div className="basis-28 text-center">{format(new Date(data_refund.tanggal), 'dd-MM-yyyy')}</div>
                            ...
                            <div className="basis-1/6 text-center">{data_refund.warehouse}</div>
    ...
    const handleValueChange = (newValue: any) => {
        if (newValue.startDate === null || newValue.endDate === null) {
            ...
            getdatarefund(startDate + " to " + lastDate, Store, Role, area, Query);
        } else {
            setDate(newValue.startDate + " to " + newValue.endDate);
            getdatarefund(newValue.startDate + " to " + newValue.endDate, Store, Role, area, Query);
        }
        setValues(newValue);
    };
    ...
    function querySet(e: any) {
        if (e.target.value === "") {
            setQuery("all");
            getdatarefund(date, Store, Role, area, "all");
        } else {
            setQuery(e.target.value);
            getdatarefund(date, Store, Role, area, e.target.value);
          ...
          >
              ...
              <fa.FaSearch size={17} className="text-gray-700" />
                ...
                <select
                    ...
                    onChange={(e) => {
                        ...
                        getdatarefund(date, e.target.value, Role, area, Query);
                    }}
            ...
            <div className="items-center content-center mb-3 mt-3  scroll-m-96">
                ...
                <div className="bg-[#323232] flex flex-row h-[40px] rounded-lg font-bold text-white items-center text-[10px]">
                    <div className="basis-14 mr-5 text-center">No.</div>
                    <div className="basis-28 text-center">Date</div>
