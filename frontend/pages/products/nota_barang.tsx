...
export default function NotaBarang() {
    ...
    const [data, setdatanota] = useState([]);
    ...
    const [data_warehouse, setdatawarehouse] = useState([]);
    ...
    useEffect(() => {
        loaddatanota(Query, Store, date, Filter_Supplier, Filter_Status, Filter_Payment);
        getbrand()
        getstore()
        getcategory()
        getwarehouse()
    ...
    async function loaddatanota(Query: any, Store: any, date: any, Filter_Supplier: any, Filter_Status: any, Filter_Payment: any) {
        setisLoading(true);
        await axios({
            method: "post",
            ...
            .then(function (response) {
                setdatanota(response.data.result.data_notabarang);
                setisLoading(false);
            })
    ...
    if (isLoading) {
        data_category.map((categorylist: any, index: number) => {
            list_category.push(
                <option key={index} value={categorylist.id_category}>
    ...
    if (isLoading) {
        data_supplier.map((supplierlist: any, index: number) => {
            list_supplier.push(
                <option key={index} value={supplierlist.id_sup}>
    ...
    if (isLoading) {
        data_warehouse.map((warehouselist: any, index: number) => {
            list_warehouse.push(
                <option key={index} value={warehouselist.id_ware}>
    ...
    if (!isLoading) {
        data.map((data_produk: any, index: any) => {
            return (
                list_produk.push(
                    <tbody key={index} className="group hover:shadow-lg rounded-lg">
                        <tr className="group-hover:drop-shadow-primary transition-filter rounded-lg">
                            ...
                            <td className="p-0 pt-4 h-full">
                                <div className="flex flex-warp gap-4 justify-center items-center h-full bg-white pt-2 md:pt-4 md:pb-[15px] px-4 pr-5 rounded-r-lg">
                                    <button
                                        onClick={() => {
        ...
        <>
            ...
            <fa.FaChevronRight size={18} className="text-gray-400 group-hover:text-gray-800" />
                ...
                <Collapse isOpened={filter}>
                    <div className="grid grid-cols-4 pb-4 pt-1 px-5 gap-3">
