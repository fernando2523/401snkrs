...
export default function PageSo() {
  ...
  useEffect(() => {
    loaddatapo(Query, date, Filter_Tipe_po, Filter_Supplier, Filter_Tipe_user);
    getsupplier();
    getusers(user_login, user_role);
    return () => { };
  }, []);

  async function loaddatapo(query: any, tanggal: any, Filter_Tipe_po: any, Filter_Supplier: any, Filter_Tipe_user: any) {
    setisLoading(true);
    await axios({
      method: "post",
      url: `https://api.401snkrs.com/v1/getpo`,
      data: {
        query: query,
        date: tanggal,
  ...
  async function getusers(user_login: any, user_role: any) {
    ...
    await axios({
      method: "post",
      url: `https://api.401snkrs.com/v1/getuserpo`,
      data: {
      ...
      .then(function (response) {
        ...
        if (
          ...
          loaddatapo(Query, date, Filter_Tipe_po, Filter_Supplier, response.data.result[0].users);
        }
        setisLoading(false);
      })
      .catch(function (error) {
  ...
  const handleValueChange = (newValue: any) => {
    if (newValue.startDate === null || newValue.endDate === null) {
      ...
      loaddatapo(Query, newValue.startDate + " to " + newValue.endDate, Filter_Tipe_po, Filter_Supplier, Filter_Tipe_user);
    }
  ...
  function querySet(e: any) {
    if (e.target.value === "") {
      setQuery("all");
      loaddatapo("all", date, Filter_Tipe_po, Filter_Supplier, Filter_Tipe_user);
    } else {
      setQuery(e.target.value);
      loaddatapo(e.target.value, date, Filter_Tipe_po, Filter_Supplier, Filter_Tipe_user);
    }
  }
  ...
  async function alldeleteData() {
    await axios
      .post(`https://api.401snkrs.com/v1/deletepo`, {
        id_po: id_po,
      })
      .then(function (response) {
        loaddatapo(Query, date, Filter_Tipe_po, Filter_Supplier, Filter_Tipe_user);
  ...
  async function deleteData() {
    await axios
      .post(`https://api.401snkrs.com/v1/deleteitem`, {
        id_act: id_act,
      })
      .then(function (response) {
        loaddatapo(Query, date, Filter_Tipe_po, Filter_Supplier, Filter_Tipe_user);
  ...
  async function showrepeatModal(
    ...
    console.log(id_act)
    await axios
      .post(`https://api.401snkrs.com/v1/get_sizepo`, {
        id_act: id_act,
      })
      .then(function (response) {
  ...
  const onSubmitedit = async (data: any) => {
    ...
    if (qty_all === 0) {
      ...
      await axios
        .post(`https://api.401snkrs.com/v1/editpo`, {
          data: data,
          id_act: id_act,
          idpo: edit_idpo,
        ...
        <tbody key={index} className="group text-xs">
          {"SUPER-ADMIN" === Cookies.get("auth_role") ||
            ...
            <>
              <tr className="">
                <td
                  className="p-0 pt-4 h-full text-sm"
                  colSpan={10}
                >
          ...
          {data_so.detail.map((item: any, index: any) => {
            return (
              <tr key={index} className="py-2">
                ...
                <td className="p-0 bg-white h-full w-[10%]">
      ...
      <div className="flex flex-row items-center content-center mb-3 gap-2">
        <div className="shadow grow rounded-lg flex flex-row text-center content-center">
          <input
            ...
            onKeyDown={keyDown}
            disabled={isLoading}
            className="h-[45px] grow text-sm border-0 pr-3 pl-5 text-gray-700 focus:outline-none rounded-l-lg"
            type="text"
            placeholder="Search ID Product Or Product Name..."
          />

          <button
            type="button"
            disabled={isLoading}
            className="rounded-r-lg bg-white basis-1/8  text-gray-700 font-medium px-5  hover:bg-gray-200"
            onClick={() => {
              // console.log(Query)
              if (Query != "all") {
                loaddatapo(Query, date, Filter_Tipe_po, Filter_Supplier, Filter_Tipe_user);
        ...
        < div className="h-[45px] grow text-sm border-0 text-gray-700 focus:outline-none rounded-l-lg" >
          <select
            value={Filter_Supplier}
            onChange={(e) => {
              setFilter_Supplier(e.target.value);
              loaddatapo(Query, date, Filter_Tipe_po, e.target.value, Filter_Tipe_user);
        ...
        < div className="h-[45px] grow text-sm border-0 text-gray-700 focus:outline-none rounded-l-lg" >
          <select
            ...
            onChange={(e) => {
              ...
              loaddatapo(Query, date, e.target.value, Filter_Supplier, Filter_Tipe_user);
            }}
            className={`appearance-none border h-[45px]  w-[100%] px-5  text-gray-700 focus:outline-none rounded-lg`}>
            ...
            <>
              <select
                ...
                onChange={(e) => {
                  ...
                  loaddatapo(Query, date, Filter_Tipe_po, Filter_Supplier, e.target.value);
        ...
        <div className="shadow grow rounded-lg flex flex-row items-center justify-end bg-white ">
          {"SUPER-ADMIN" === Cookies.get("auth_role") || "HEAD-AREA" === Cookies.get("auth_role") ? (
            <>
              <Datepicker
                ...
                placeholder={startDate + "-" + lastDate}
                readOnly={true}
                ...
                <div className="border-0 rounded-lg shadow-lg flex flex-col bg-white outline-none focus:outline-none w-[100%]">
                  ...
                  <div className="flex items-start justify-between px-5 py-3 border-b border-solid border-slate-200 rounded-t">
                    <span className="text-base font-semibold">
                      Edit Qty Report Order : {edit_produk} | {gudang} | Code : {id_act} <span hidden>{edit_id_ware}</span>
                    </span>
                  </div>
                  ...
                  <div className="px-6 gap-4 flex flex-auto h-[auto]">
                    <div className="text-sm w-full pb-10 flex flex-col">
                          ...
                          <tr className="">
                            ...
                            {"SUPER-ADMIN" === Cookies.get("auth_role") ||
                              ...
                              <>
                                ...
                                <th className="py-2">Total Stock Before</th>
                              ...
                              <>
                                <th className="py-2 rounded-l-lg">Size</th>
                                <th className="py-2">Total New Stock</th>
                                {/* <th className="py-2 rounded-r-lg">
                                  Total SO Baru
                                </th> */}
                              </>
                      ...
                      <div className="h-[10%] mt-6 gap-4 w-full flex flex-row items-end justify-start">
                        <button
